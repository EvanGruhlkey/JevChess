...
def test_browser_page_and_assets_are_served(tmp_path):
    ...
    assert b'data-mode="jev-vs-jev"' in page.data
    assert b'>Jev vs Jev</button>' in page.data
    ...
    assert b"SPECTATOR_DELAY_MS" not in script.data
    assert b"setTimeout(() => requestJev" not in script.data
